...
interface NewPaymentProps {
  pageName: 'Payment' | 'Transaction';
}

export const NewPaymentComponents: React.FC<NewPaymentProps> = ({
  pageName,
}) => {
  ...
  const getToken = async () => {
    try {
      ...
      if (res) {
        ...
        setUrl(
          `${companyUrl}/Account/AutoLogin?ForUserId=${UserInfo?.userID}&returnPage=${ReturnPage[pageName]}`,
        );
