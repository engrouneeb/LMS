...
export const SystemReports: React.FC<SystemReportsProps> = ({}) => {
  ...
  const getToken = () => {
    AsyncStorage.getItem('@UserAuth').then((res) => {
      if (res != null) {
        ...
        setUrl(
          `${UserInfo.companyUrl}/Account/AutoLogin?returnPage=${ReturnPage['Reports']}`,
        );
        console.log('====================================');
        console.log(
          `${UserInfo.companyUrl}/Account/AutoLogin?returnPage=${ReturnPage['Reports']}`,
  ...
  return (
    <_Screen
      header={<Header isBack goBack={handleBack} Screen={'System Reports'} />}
